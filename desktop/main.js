 ...
 *
 * No URL or API-key entry is shown to regular users.
 * Self-hosters can override the server via Tray → Settings.
 */
...
// ── Server URL ───────────────────────────────────────────────────────────────
// Set LUCA_SERVER_URL before running electron-builder to bake a custom URL in.
// Defaults to the public LUCA deployment.
const DEFAULT_SERVER_URL = process.env.LUCA_SERVER_URL || 'https://goluca.ai'

// ── Persistent config (only stores optional server override + window state) ──
const CONFIG_PATH = path.join(app.getPath('userData'), 'luca-config.json')
...
function openMainWindow () {
  ...
  // If the server URL can't be reached show a clean "enter your server URL"
...
<div class="card">
  <div class="hex">⬡</div>
  <h1>LUCA</h1>
  <p>Could not connect to your LUCA server.<br>Enter the URL where your server is running.</p>
  <label>Server URL</label>
  <input id="url" type="url" placeholder="https://your-server.replit.app" value="${currentUrl}">
...
<script>
  async function connect() {
    ...
    try { new URL(url) } catch(_) { err.textContent = 'That doesn\\'t look like a valid URL.'; return }
    btn.textContent = 'Connecting…'
    btn.disabled = true
    err.textContent = ''
    const result = await window.lucaDesktop.saveServerUrl(url)
    if (result && result.ok === false) {
      err.textContent = result.error || 'Could not save URL.'
...
function _buildTrayMenu (connected, updateState) {
  ...
  template.push({ label: 'Server Settings…', click: openSetupWindow })
  template.push({ type: 'separator' })
...
// ── Setup window (Server Settings — for self-hosters only) ───────────────────
...
// Settings IPC (used by setup.html)
ipcMain.on('get-config', (e) => {
  e.returnValue = { serverUrl: config.serverUrl || '', defaultUrl: DEFAULT_SERVER_URL }
})

ipcMain.handle('save-server-url', (_, url) => {
  try {
    const parsed = new URL(url)
    if (!SAFE_SCHEMES.includes(parsed.protocol)) return { ok: false, error: 'URL must start with https://' }
